...
def main():
    data = RealDataset(data_path=f"Data/Mozarteum/partitions_mozarteum/excerpts/fold_2/test_photoscore.txt", 
    ...
    model = SMTPP_Trainer.load_from_checkpoint('weights/finetuning/SMTPP_Mozarteum_Bekern_f2.ckpt').model
    ...
    with torch.no_grad():
        for batch in loader_test:
            ...
            gt = "".join([model.i2w[token.item()] for token in y.squeeze(0)[:-1]])
            gt = gt.replace("<t>", "\t")
            gt = gt.replace("<b>", "\n")
            gt = gt.replace("<s>", " ")
